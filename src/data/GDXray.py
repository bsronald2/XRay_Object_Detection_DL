...
class GDXray:
    ...
    def __init__(self, input_dir, train_val_ds=False):
    ...
    def load_dataset(self):
        ...
        """
        self.logger.info(f'images size:{len(list(self.input_dir.glob("*/*.png")))}')
        data_set = self.__load_batch(str(self.input_dir.absolute() / '*.png'))
    ...
    def pre_process(self, output_dir):
        """ Make..."""
        p_raw_ds = self.__load_batch(str(self.input_dir / '*.png'), shuffle_buffer_size=GDXray.BUFFER_SIZE)
        final_dir = PurePath(str(self.input_dir)).name
